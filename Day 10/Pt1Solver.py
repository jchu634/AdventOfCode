...
def get9locations(value, location, grid, maxX, maxY):
    if value == 9:
        return {location}
    ...
    def check_and_update(dx, dy):
        ...
        if 0 <= new_x <= maxX and 0 <= new_y <= maxY and grid[new_y][new_x] == value + 1:
            locations.update(get9locations(
                value + 1, (new_x, new_y), grid, maxX, maxY))
...
def getAns():
    ...
    for trailhead in trailHeads:
        temp = len(get9locations(0, trailhead, grid, maxX, maxY))
        # logging.info(f"{trailhead}: {temp} {
        #              get9locations(0, trailhead, grid, maxX, maxY)}")
        total += temp
